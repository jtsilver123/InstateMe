...
(function () {
    ...
    const revealTargets = document.querySelectorAll('.reveal, .fin__chart, .model, .ask__bridge, .bars, .model__bars');
    ...
    if (form) {
        ...
        const mailtoFallback = (name, email, thesis, pof) => {
        ...
        form.addEventListener('submit', (e) => {
            ...
            if (!name || !email) {
                const miss = form.querySelector(`input[name="${name ? 'email' : 'name'}"]`);
                if (miss)
                    miss.focus();
                return;
            }
    ...
    (function calculator() {
        const capEl = byId('capture');
        if (!capEl)
            return;
        const marginEl = byId('margin');
        const txEl = byId('txBase');
        const chipsWrap = byId('stateChips');
        if (!marginEl || !txEl || !chipsWrap)
            return;
